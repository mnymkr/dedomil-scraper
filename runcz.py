...
def has_cz_version(soup):
    """Check if any model name contains 'CZ' in MODELS divs."""
    for model_div in soup.find_all("div", class_="MODELS"):
        if "CZ" in model_div.get_text():
            return True
    return False
...
def scrape_game(game_id):
    ...
    try:
        ...
        # New rule: skip games with no CZ version at all
        if not has_cz_version(soup):
            print(f"No CZ version found for {game_id} — skipping")
            return f"{game_id}: skipped (no CZ version)"
